...
// Consulta para obtener los préstamos según la opción seleccionada
                ...
                <div class="row">
                    ...
                    <table class="table table-bordered table-prestamos">
                        ...
                        <tbody>
                            ...
                            function obtenerFechaDevolucionReal($codPrestamo) {
                                $dbhost = "localhost:3308";
                                $dbuser = "root";
                                $dbpass = "";
                                $dbname = "bibliotecaprueba";
                                ...
                                $query = "SELECT fechaDevolucionReal FROM Prestamos WHERE CodPrestamo = $codPrestamo";
                                $result = mysqli_query($conn, $query);

                                if ($result && mysqli_num_rows($result) > 0) {
                                    $row = mysqli_fetch_assoc($result);
                                    return $row['fechaDevolucionReal'];
                                } else {
                                    return 'Fecha no disponible';
                                }
                            }
    ...
    <?php include_once 'Modales/modalDevueltos.php'; ?>
    ...
    <script>
        ...
        function guardarNuevaFecha() {
            ...
            if (nuevaFecha !== null && nuevaFecha !== "") {
                // Realiza una solicitud AJAX para actualizar la fecha de devolución en el servidor
                $.ajax({
                    type: 'POST',
